...
<script>
import {error, hasOwn, _toString} from '@flyer-ui/commonality'
export default {
  name: 'mForm',
  props: {
    value: {
  ...
  methods: {
    ...
    getItems () {
      this.items = this.$slots.default.filter(VNode => {
        return VNode.tag && VNode.componentInstance && (VNode.componentOptions.tag === 'm-form-item')
      })
      ...
      if (typeof callback === 'function') {
        if (Object.keys(this.errors).length > 0) {
          ...
          for (const error in this.errors) {
            ...
            const message = this.errors[error][0].message
            item && item.showError(message)
            messages.push(message)
          }
          callback.call(null, false,messages,this.errors)
        } else {
          // eslint-disable-next-line no-useless-call
          callback.call(null, true,this.errors)
        }
      }
    },
    resetForm () {
